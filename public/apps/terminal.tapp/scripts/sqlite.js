...
/**
 * CLI for the SQLite subsystem via Dusk runtime.
 * @param {argv} args
 * @param {Terminal} term
 */
async function sqlite(args, term) {
	...
	const database = args._ && args._[1] ? String(args._[1]) : ":memory:";

	displayOutput(`Opening SQLite database: ${database}...`);
	setTabTitle("SQLite");

	try {
		const process = await window.parent.tb.dusk.sqlite.spawn(database, {
		...
		displayOutput(`\r\nSQLite exited with code ${exitCode}`);
		createNewCommandInput();
	} catch (error) {
		console.error("[Terminal] SQLite spawn error:", error);
		displayOutput(`\r\nError: ${error.message}`);
		window.parent.tb.setCommandProcessing(true);
		createNewCommandInput();
	}
